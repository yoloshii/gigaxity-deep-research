...
The motivation is structural rather than incremental coverage. Everything else in the general-web lane is either scraped or metered. SearXNG's engines scrape Google-adjacent providers from the instance's own IP, so under sustained automated load they get CAPTCHA'd and rate-limited — and they fail *silently*, returning HTTP 200 with degraded or off-topic results rather than an error (see the two troubleshooting rows added in v0.9.2). Measured on a live instance during this release cycle: `brave` "Suspended: too many requests", `duckduckgo` CAPTCHA after roughly six rapid queries, `startpage` CAPTCHA, leaving Bing to serve blocked-client fallback pages into the fusion. Brave's API is a keyed lane over Brave's own independent index, so it cannot be served a bot-block page at all; it is the one general-web source in the stack whose availability does not degrade with query volume.

Implementation mirrors the existing connectors: `BraveConnector(Connector)` with `search()`, `is_configured()`, and a `_probe_url()` pointing at the API root so `GET /api/v1/health/connectors` proves reachability without spending a billed query. No official Python SDK exists, so it speaks HTTP via `httpx` in the same shape as the SearXNG connector. Three details are load-bearing: `count` is clamped to Brave's documented maximum of 20 rather than forwarding whatever `top_k` a caller passes (Brave rejects higher values); a response carrying no `web` key — which is what Brave returns when it answers a query only with videos or news — is treated as zero results rather than an `AttributeError`, so it fuses as a quiet non-contributor; and blank optional parameters are omitted from the query string instead of being sent as empty strings. Source IDs use the `br_` prefix, and Brave's `description` field maps to the `Source.content` contract.

New `tests/test_brave_connector.py` (16 tests): live-response-shape mapping, `br_` ID prefixing and distinctness, rank-descending scores, the absent-`web` and null-`web` paths, count clamping at both ceiling and floor, the key travelling in `X-Subscription-Token` and never in the query string, blank-parameter omission, country forwarding, probe-URL-is-not-the-billed-path, no-network-when-unconfigured, and exception absorption so a failing connector degrades to zero sources instead of taking down the gather. Suite: 647 passed / 52 skipped (631 + 16 new), no regressions.

MINOR (0.9.x → 0.10.0): a new connector, three new `RESEARCH_BRAVE_*` settings, and a changed default connector set are additive public contract. No existing endpoint, citation shape, verifier gate, or cache key changes. Docs updated in parity: `.env.example`, `docker-compose.yml`, `docs/reference/configuration.md`, `SKILL.md`, the `search` tool docstring, and the env tables plus the connectors-are-parallel-not-failover anti-pattern in `CLAUDE.md` / `AGENTS.md` (kept byte-identical). Applied to `main` and `local-inference` in parity.
...
The example config had a structural bug that made it quietly not do what it looked like it did. It opened with a bare `use_default_settings: true` followed by a tiered `engines:` block — which reads as a curated set of ~14 engines, but upstream merges an `engines:` block into the ~250 stock engines **by name**, so every engine the file never mentions keeps its default state. On a live instance built from this template that worked out to 92 enabled engines, 79 of which the file never named. Most sat in categories a web query never touches, but five reached `general`, including `aol` — default-enabled, in the general pool, and returning HTTP errors. The file now uses `use_default_settings.engines.keep_only`, upstream's actual allow-list construct, so omission removes rather than silently permits. Documented at `docs.searxng.org/admin/settings/settings.html#use-default-settings`; the merge-by-name semantics are stated there explicitly.

Two engine entries asserted upstream states that had since changed. `google news` was disabled citing issue [#5852](https://github.com/searxng/searxng/issues/5852) with PR [#5984](https://github.com/searxng/searxng/pull/5984) as a blocked draft POC — that PR merged and the issue closed as completed on 2026-05-17, so the stated reason had been false for eleven weeks. `karmasearch` (plus its images/videos/news variants) was disabled "until a durable fix ships upstream"; there will not be one, because the engine was **deleted** from SearXNG in PR [#6213](https://github.com/searxng/searxng/pull/6213) on 2026-06-07 after Cloudflare 403s proved unbeatable from every tested IP including Tor exits ([#5976](https://github.com/searxng/searxng/issues/5976)). Four entries naming a removed engine are dead config, not disabled engines, and are gone.

`bing` is now disabled by default, and dropped from `RESEARCH_SEARXNG_ENGINES` in `src/config.py`, `.env.example`, `docker-compose.yml`, and the env tables in `CLAUDE.md` / `AGENTS.md` / `docs/reference/configuration.md`. Measured cause: asked for RAG evaluation benchmarks, Bing returned "Microsoft Official Home Page", "Sign in to your account", "Outlook" and "Microsoft 365"; asked something else it returned unrelated Chinese forum threads about CPU temperatures. That is Bing's blocked-client response, served at HTTP 200 with no error field. Because RRF fuses on rank, junk at any weight displaces good results, so deweighting is not a remedy — the entry stays in `keep_only` with a copy-pasteable test so operators whose egress Bing tolerates can re-enable it in one flag.

The engine set gains `semantic scholar`, `pubmed`, `openairepublications`, `mdn`, `askubuntu`, `superuser` and `pypi` — all key-less, all previously running unmanaged as defaults, all appropriate for a research backend — and `wikidata` moves from disabled to enabled. Two operational sections are new. **Health check**: a degraded SearXNG returns HTTP 200, never an error, so the `unresponsive_engines` field is the only signal that engines are CAPTCHA'd or suspended; the file shows the one-line query for it and the threshold worth alerting on. **Durable lanes**: scraped engines lose the anti-bot arms race structurally — upstream has now removed engines outright over it, and got Google working only through a Playwright service carrying a signed-in session cookie that, per the maintainer, "expires really quickly" ([discussion #5651](https://github.com/searxng/searxng/discussions/5651)) — so an official-API lane (Brave Search API; or `use_official_api` + `api_key` on qwant/mwmbl/core/tagesschau) is documented as the answer for sustained automated load.

`limiter` stays `false`, and now says why: it throttles traffic **inbound** to your instance, to stop other people's bots riding it and burning your server IP's reputation. For a private JSON-API backend the only client is your own connector, so enabling it would rate-limit exactly the traffic the instance exists to serve — pace on the caller side instead, and set it true the moment the instance is publicly reachable.

MINOR (0.8.x → 0.9.0): a changed default in `RESEARCH_SEARXNG_ENGINES` and a restructured recommended configuration are consumer-visible, matching the class used for v0.8.0. No Python source changed beyond that one default; the six MCP tools, the five STRUCTURAL verifier gates, the relevance gate, citation shapes, and cache keys are untouched. Applied to `main` and `local-inference` in parity.
...
Brings Jina in-repo as a bundled companion: [`companions/jina-mcp/`](companions/jina-mcp/), a single-file Python MCP server exposing 19 tools. The recommended `jina` config in every doc changes from `"type": "http"` against `https://mcp.jina.ai/v1` to stdio against the bundled server. Tool names are unchanged, so `mcp__jina__*` references in the bundled skill, `CLAUDE.md`, and any downstream routing keep resolving — no consumer edits required.

The hosted server had become unusable on exactly the tier this project recommends. Its entire search family POSTs to `svip.jina.ai` (`src/utils/search.ts:72,108,142,276` upstream), a paid lane that answers a trial-credit key with `HTTP 500 {"error":"Not enough credits"}`. Because the hosted worker returns `response.statusText` and discards the body, what reached callers was a bare `Internal Server Error` — a billing condition indistinguishable from an outage. From ~1 Aug 2026 this took out `search_web`, `parallel_search_web`, `search_arxiv`, `search_ssrn`, and `search_images` for every free-10M-tier user, while `read_url`, the rerank/embeddings tools, and `extract_pdf` kept working, which made the failure read as partial and intermittent rather than lane-shaped. Tracked upstream at [jina-ai/MCP#32](https://github.com/jina-ai/MCP/issues/32) and [jina-ai/reader#1258](https://github.com/jina-ai/reader/issues/1258), both open and unacknowledged at time of writing. Key rotation is not a remedy and is worth stating plainly: a replacement key is another trial key, and the lane refuses trial credits by design — verified across nine keys, two of which held millions of unspent trial tokens and were refused identically, while genuinely exhausted keys returned a *different* string (`run out of its token`). The two conditions are distinguishable, so the bundled server distinguishes them: `show_api_key` reports both balances, and every failure path names what the server actually said.

Three routing decisions differ from the hosted implementation. **Web search targets `s.jina.ai`**, which accepts trial credits; its validator rejects `num > 20` while the hosted server declares a default of 30, so the bundled server never sends `num` at all — it takes the default page and slices client-side, making the cap unreachable from the tool surface regardless of what a caller passes. **Academic search leaves Jina entirely.** The hosted server scoped arXiv and SSRN with a `domain:` body param that only `svip` interprets; `s.jina.ai` accepts the param and silently ignores it, returning general web results (confirmed by a control query returning byte-identical results with and without it) — so repointing alone would have produced plausible, wrong answers rather than an error. `search_arxiv` now uses the arXiv export API and `search_ssrn` uses OpenAlex filtered to the SSRN source, both free and key-less, with `search_bibtex` on DBLP → Semantic Scholar as the hosted server already did. This is a capability gain, not parity: `search_arxiv` accepts native arXiv field syntax the wrapper never exposed (`cat:cs.CL AND abs:"retrieval augmented"`, `au:Vaswani`) and a `sort="date"` mode for newest-first, which matters for literature review. **Every error surfaces the response body**, with lane-aware guidance attached — the discarded-body bug is the single reason the original outage took days to diagnose rather than minutes.

Two tools degrade deliberately rather than silently. `search_images` has no free-lane equivalent (`s.jina.ai` returns 503 for `type: "images"`), so it stays on `svip` and returns an explanatory error without paid balance; `search_jina_blog` needs an optional `JINA_GHOST_KEY` and says so when unset. Two are not carried over: `expand_query` (12k tokens/call, already flagged AVOID throughout the docs) and `deduplicate_images`. One upstream breakage is out of scope for any client: site-restricted search fails regardless, because `s.jina.ai` proxies both the `X-Site` header and the `site:` operator to the same degraded backend — the `site` argument is wired and documented as currently-broken rather than removed, so it starts working when Jina fixes it.

MINOR (0.7.x → 0.8.0): a new bundled component and a changed recommended configuration are additive-but-consumer-visible. The parent server's own contract is untouched — six MCP tools, the five STRUCTURAL verifier gates, the relevance gate, citation shapes, and cache keys are all unchanged, and no Python source under `src/` was modified. Docs updated in parity: `docs/guides/setup-companions.md` (full install section, table row, order list — a user following that guide previously never installed the companion at all), `companions/README.md` (Jina moves out of "Not bundled"), `docs/guides/triple-stack-setup.md` (Stack 4 config plus three new troubleshooting rows), `docs/reference/mcp-configs.md` (config block and corrected tool inventory), `README.md` (step 5 now installs before registering; the stack table marks Jina bundled), `docs/guides/free-tier-strategy.md` (corrects a claim that became false — the hosted free tier does not cover search, and arXiv/SSRN/BibTeX now cost nothing against the Jina allowance at all), `docs/concepts/fallback-chains.md` and `companions/gptr-mcp/README.md` (two further `site:`-scoped routes the first sweep missed), and `CLAUDE.md` / `AGENTS.md` (kept byte-identical). Verified against a live trial key: 19/19 tools respond, 17 fully functional and 2 returning their documented explanatory errors, over a real stdio MCP handshake.

## v0.7.1 (2026-08-01)

Dependency fix — pins `fastmcp>=3.3,<3.4` (was `>=0.1.0`). The stdio MCP server and the test suite both target FastMCP 3.x semantics — since v0.5.x the `@mcp.tool()` decorator is expected to leave the original coroutine bound at the module name, and the tool-wiring tests resolve tools by module attribute on that basis (`tests/test_cache.py` `_tool_fn`). Under the old floor-only pin, an environment resolving fastmcp 2.x (where the decorator replaces the module attribute with a non-callable `FunctionTool`) made a fresh install look broken: **19 tool-wiring test failures** (`'FunctionTool' object is not callable`) across `test_cache.py`, `test_codex_t5_citation_extraction.py`, and `test_codex_t8_v030_citation_unification.py` — the same fresh-clone-looks-broken class v0.6.4 fixed for the missing-key case. The upper bound is `<3.4` deliberately: fastmcp 3.4.x moves to starlette ≥1.x, which conflicts with `fastapi<0.119`'s `starlette<0.49` requirement, so a joint resolve either backtracks anyway or breaks the REST stack; 3.3.x is the verified line (3.3.1 is what the suite and server are exercised against). No code change; no public-contract change beyond the dependency constraint — PATCH. Keyless suite on a conforming environment: 631 passed / 52 skipped, zero failures. Identical on `main` and `local-inference`.

## v0.7.0 (2026-08-01)

Adds a **connector liveness surface**: `GET /api/v1/health/connectors`. Until now the server's only notion of connector health was config presence — `/health` lists `get_active_connectors()`, which filters on `is_configured()` (an env-var check), and a configured-but-dead backend was discovered only when a live query failed inside `SearchAggregator.search()` and was logged-and-dropped from fusion. The three states that matter — key/host present, endpoint actually answering, and neither — were indistinguishable from outside. The new endpoint really contacts each connector in parallel (`asyncio.gather` with per-connector isolation so one misbehaving probe cannot take down the report) and classifies each as `ok` / `unreachable` / `unconfigured` (plus `error` for a probe that itself failed), with per-probe latency on every configured probe — successes and failures alike. Each probe runs under a true **wall-clock deadline** (default 2s, via `asyncio.timeout`) — httpx's own timeout is per-phase inactivity, so without the outer deadline a slow-drip response could hold the unauthenticated request open far longer — and **redirects are not followed**: a 3xx already proves liveness. The doctor enforces the deadline independently as well (with a 1s grace), so even a connector whose `health()` override ignores `timeout_s` cannot stall the report — it becomes that connector's `error` row. SearXNG is probed at its documented `/healthz`; Tavily and LinkUp at their public API roots — for those two, `ok` proves **reachability, not credential validity** (validating a key would spend a billed call, so it is deliberately out of scope and stated in the schema docstring). Unconfigured connectors are still reported: surfacing all three states side by side is the point. Probe failure details carry the exception **type** only, never the message — httpx error messages embed the probed URL, and a SearXNG host may be an internal address.

Implementation: a `ConnectorHealth` dataclass plus a default `Connector.health()` (driven by a per-connector `_probe_url()`) in `src/connectors/base.py`, a new `src/connectors/doctor.py` (`check_connectors()`), `ConnectorHealthEntry`/`ConnectorHealthResponse` schemas, and the route. `status` is a typed `Literal["ok", "unreachable", "unconfigured", "error"]` shared between the dataclass and the OpenAPI schema, so the finite state set is part of the published contract. The existing `/health` endpoint is byte-identical and still performs zero network I/O — a regression test pins its exact legacy shape. New `tests/test_connector_health.py` (20 tests: the full three-state report side by side, a hanging `health()` override that cannot stall the gather, any-HTTP-status-is-alive, redirect-not-followed (a 3xx is `ok`), timeout/connect-error classification, the wall-clock deadline cutting a slow drip, failure latency, parallel-not-serial execution, no-network guarantees for the unconfigured and no-probe paths, the URL-never-in-detail output-boundary regression, doctor isolation with an exploding connector, probe-URL contracts for all three connectors, endpoint schema, and the `/health` exact-legacy-shape no-network regression).

This is a MINOR (0.6.x → 0.7.0): a new public REST endpoint and two new response schemas are additive public contract. No existing endpoint, citation shape, verifier semantics, cache key, or output changes; the five STRUCTURAL verifier gates, the relevance gate, and the MCP tool surface (six tools) are untouched — the doctor is deliberately REST-only. Applied to `main` and `local-inference` in parity; the connector layer is identical on both branches.

## v0.6.5 (2026-07-31)

Documentation addition — no code change. The bundled `skills/research-workflow/SKILL.md` gains a **dead-repo recovery** procedure covering a failure mode the skill previously had no answer for: the search succeeded but the *result* is dead. Search indexes serve deleted, renamed, transferred and private repositories — a GitHub index was observed returning entries that 404 within hours of being surfaced — so a `404` on clone means the index is stale, not that the project is gone. Three coordinated additions: the **Repository Discovery** fallback chain gains a branch distinguishing a failed *search* from a dead *result*, with the recovery ladder inline; new **Pattern 17** documents the full chain with runnable commands (owner-account probe → same-account sweep → name search → exact-description search), identity verification (README match plus `git log --reverse` history predating the disappearance), metadata **re-baselining** from the clone rather than the index snapshot, and a four-way outcome taxonomy (RECOVERED / REPLACED / DEAD / PRIVATE) so a later pass neither re-surveys settled ground nor loses the gap; and new **Anti-Pattern 7** carries six wrong/right pairs at the point of failure.

Two findings drive the specifics. The **exact-description search is the discriminator, not the name** — generic repository names collide heavily (one dead repo returned 37 same-name hits, none of them the project, while its description phrase returned exactly one, under a different owner). And a **404 on the owner account itself** means the account was renamed or transferred, so the project is most likely alive under a new owner with the same repository name — a case the name search alone resolves only by accident. Re-baselining matters because index snapshots drift badly: one recovered project was indexed as "0 stars, new" and was in fact 191 commits with 64 stars, pushed the same day, which moved it across candidate tiers. Field result across one discovery pass: four dead candidates → two fully recovered (both top-tier), one functionally replaced, one genuinely gone.

No public-contract change — no API schema, citation shape, verifier semantics, cache key, or output change; no routing or tool-selection change; the five STRUCTURAL verifier gates and the relevance gate are untouched — so this is a PATCH. The skill's own frontmatter `version` stays at 1.0.0 per existing convention (it has been stable across v0.5.2, v0.6.0 and v0.6.2, which all edited this file). Applied to `main`; **`local-inference` still needs the same patch to restore the documented byte-identity of the bundled skill across branches.**

## v0.6.4 (2026-07-19)

Fixes a first-run failure that made a fresh clone look broken: with no `RESEARCH_LLM_API_KEY` set, `pytest` reported **19 failures**. The cause was not the tests — the OpenAI SDK raises `OpenAIError: Missing credentials` at client **construction** on an empty key, and `OpenRouterClient.__init__` builds its `AsyncOpenAI` eagerly, so every object owning a client became un-constructible. Pure unit tests were the visible casualty: `tests/test_synthesis.py::TestSynthesisEngine::test_init_defaults` is marked `@pytest.mark.unit`, takes no fixture, and does nothing but `SynthesisEngine()` before asserting two float attributes — it raised on the first line. The same root cause took down request-validation and tool-signature tests in `test_api.py`, `test_cache.py`, `test_codex_t8_v030_citation_unification.py`, `test_e2e.py`, `test_enhanced_endpoints.py`, and `test_synthesis.py`. Construction was never the intended guard: `settings.require_llm_key()` already fail-fasts at the MCP and REST entrypoints (`run_mcp.py` calls it on import), which is where a missing key should surface. `src/llm_client.py` now passes a `"not-configured"` placeholder when no key is set, so the client constructs and a real request fails with a normal upstream 401 instead of an import-time crash.

Note that setting a dummy key was **not** an equivalent workaround: it satisfies the `live_llm` fixture guard in `tests/conftest.py`, which un-skips 31 live-LLM tests that then fail against the fake credential (18 failed / 624 passed / 21 skipped). The placeholder here is internal to the client and leaves `RESEARCH_LLM_API_KEY` unset, so those tests keep skipping correctly. Keyless suite goes from **19 failed / 590 passed / 52 skipped** to **611 passed / 52 skipped**, with the skip count unchanged and every previously-passing test still passing (590 + 19 + 2 new = 611).

No public-contract change — no API schema, citation shape, verifier semantics, cache key, or output change; the only observable difference is that a misconfigured deployment now surfaces its missing key at the entrypoint guard or as an upstream 401 rather than as a construction-time exception — so this is a PATCH. Also adds two integration tests (`tests/test_gate_fail_open.py`) covering REST fail-open on `/synthesize/enhanced` and `/synthesize/p1`: both endpoints already routed a PARTIAL-with-zero-good gate decision through `apply_fail_open()` so synthesis proceeds over set-aside sources with a low-relevance caveat instead of refusing, but that behavior shipped untested end-to-end. Identical on `main` and `local-inference`.

## v0.6.3 (2026-07-05)

Adds a source-**evidence discipline** to the synthesis prompt surface: rank sources by authority (primary/authoritative sources — official docs, filings, changelogs, peer-reviewed papers, standards — carry factual claims, while forum/social/community sources are treated as community-sentiment evidence, weak for a factual claim unless corroborated); separate established fact from source-backed inference from unresolved uncertainty; and flag single-source or disagreeing claims instead of implying consensus. The discipline lives in ONE shared `EVIDENCE_DISCIPLINE` constant in `src/synthesis/citations.py` (mirroring how `CITATION_FORMAT_GUIDE` is shared) and is injected across all three synthesis prompt families, because `synthesize` and `reason` do **not** use `RESEARCH_SYSTEM_PROMPT` — they route through the aggregator style prompts and the outline SECTION/CRITIQUE/REFINE pipeline. So the fragment lands in `RESEARCH_SYSTEM_PROMPT` (the `research` tool), the aggregator COMPREHENSIVE/COMPARATIVE/ACADEMIC/REASONING prompts (`synthesize` non-outline presets + `reason`), and the outline SECTION/REFINE prompts (`synthesize` comprehensive/academic/tutorial presets), with matching checks added to the outline CRITIQUE prompt; the CONCISE style (the `fast` preset) gets a compressed one-line form (`EVIDENCE_DISCIPLINE_BRIEF`) so it stays terse. `OUTLINE_PROMPT` (headings only) is untouched. In `RESEARCH_SYSTEM_PROMPT` the now-subsumed "Note when information is limited or uncertain." line is removed and the recency line reworded ("Prefer recent authoritative sources when timeliness matters."), since authority now belongs to the fragment and recency does not.

No public-contract change — no API schema, citation shape, or verifier `passed` / `soft_warnings` semantics change; the five STRUCTURAL verifier gates and the relevance gate are untouched — so this is a PATCH. `SYNTH_CACHE_VERSION` is bumped (5 → 6) because the new prompt changes synthesis prose for the same inputs, so pre-v6 cached syntheses are not served. Adds `tests/test_evidence_discipline.py` (twelve tests: presence across every final prose surface, absence from `OUTLINE_PROMPT` and the CONCISE full-block negative controls, brace-safety `.format()` regression, and the cache bump); the existing citation-guide-needle regression (`test_codex_t7_v022_fixes.py`) passes unchanged. Cleared by codex GPT-5.5 high adversarial DESIGN review (session `019f32a9`) across design and fix turns to verbatim "Zero remaining findings"; the unit suite passes with no new failures on the existing baseline (the deselected real-API integration tests need a live LLM key). The `EVIDENCE_DISCIPLINE` constant and all injected prompts are byte-identical on `main` and `local-inference`.

## v0.6.2 (2026-07-05)

Documentation addition — no code change. The bundled `skills/research-workflow/SKILL.md` gains a **"Framing a deep-research query (EXPLORATORY / SYNTHESIS)"** section covering the caller-side discipline the skill previously left implicit: how to shape the query string handed to `discover` / `synthesize` before classification. It states that the engine has no conversation memory — it acts only on the query text and the sources passed to it — and gives five rules: lead with the goal and the decision it informs; embed all context (names, dates, versions, ruled-out options) in the query string; declare the source hierarchy when epistemics matter (primary/authoritative sources carry claims, forum/social results are weak signal only, never sole support for a fact); name include/avoid constraints; one mission per query. It notes that the engine already decomposes into typed sub-aspects, detects gaps, and surfaces contradictions, so a well-framed query is what makes those stages fire on the right axes. No routing, tool, or verdict-handling change; the five STRUCTURAL verifier gates and the relevance gate are untouched. Identical on `main` and `local-inference` (the bundled skill is byte-identical across branches).

## v0.6.1 (2026-06-25)

Documentation alignment for the v0.6.0 gate fail-open change — no code change. Three overview docs still carried the pre-fail-open absolute framing: the README quality-gate feature bullet said the gate "keeps low-quality sources out of synthesis"; `docs/concepts/architecture.md` said scoring "rejects sources below a quality threshold before they enter the LLM context"; and `docs/concepts/presets.md` described a sub-threshold average as one that "rejects the whole set". All three now note that a REJECT **fails open** over the set-aside sources with a low-relevance caveat — when any source clears `RESEARCH_FAIL_OPEN_MIN_SOURCE_SCORE` (default 0.3) — instead of refusing. The primary v0.6.0 behavior docs (`docs/reference/mcp-tools.md`, the bundled `skills/research-workflow/SKILL.md`, `CLAUDE.md`/`AGENTS.md`, `.env.example`, `docs/reference/configuration.md`) were already correct; `docs/reference/rest-api.md` and `docs/troubleshooting.md` describe the post-synthesis output verifier (unchanged) and are untouched. Identical on `main` and `local-inference`.
