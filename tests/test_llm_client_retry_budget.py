...
* the endpoint, key and model reach the client from settings rather than from a
  literal — this branch points at any OpenAI-compatible server, so a hardcoded
  endpoint would silently defeat the reason the branch exists.
...
def test_retry_count_is_configurable(monkeypatch):
    ...
    assert LLMClient(api_key="test-key")._client.max_retries == 0
